...
def lambda_handler(event, context):
    ...
    try:
        ...
        try:
            ...
            if tool_name == "get_security_service_costs":
                result = loop.run_until_complete(
                    server_instance._get_security_service_costs(arguments)
                )
            elif tool_name == "analyze_cost_trends":
                result = loop.run_until_complete(
                    server_instance._analyze_cost_trends(arguments)
                )
            elif tool_name == "get_cost_breakdown":
                result = loop.run_until_complete(
                    server_instance._get_cost_breakdown(arguments)
                )
            elif tool_name == "forecast_costs":
                result = loop.run_until_complete(
                    server_instance._forecast_costs(arguments)
                )
            else:
                return {
                    'statusCode': 400,
