"""Lambda handler wrapper for Security MCP Server"""
import json
import asyncio
import logging
...
def lambda_handler(event, context):
    """AWS Lambda handler for Security MCP Server"""
    try:
        ...
        try:
            ...
            if tool_name == "check_security_services":
                result = loop.run_until_complete(
                    server_instance._check_security_services(arguments)
                )
            elif tool_name == "get_security_findings":
                result = loop.run_until_complete(
                    server_instance._get_security_findings(arguments)
                )
            elif tool_name == "check_compliance":
                result = loop.run_until_complete(
                    server_instance._check_compliance(arguments)
                )
            else:
                return {
                    'statusCode': 400,
